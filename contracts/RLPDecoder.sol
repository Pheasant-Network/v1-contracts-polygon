...
library RLPDecoder {
    ...
    uint256 constant LIST_LENGTH = 20;
    ...
    function decodeList(bytes calldata input) internal pure returns (bytes[] memory) {
        ...
        if (firstByte <= hex"f7") {
            ...
            uint256 llength = uint8(firstByte) - uint8(bytes1(hex"f7"));
    ...
    function decodeBytes(bytes memory input) internal pure returns (RLPItem memory item) {
        ...
        if (firstByte <= hex"7f") {
            ...
            uint256 length = uint8(firstByte) - uint8(bytes1(hex"7f"));
            bytes memory data;
            if (firstByte == hex"80") {
                data = bytes("");
                item = RLPItem(data, 1);
            ...
            uint256 llength = uint8(firstByte) - uint8(bytes1(hex"b7"));
            require(input.length > llength);
            uint256 length = toUintX(slice(input, 1, llength), 0);
            require(length > 55);
            item = RLPItem(slice(input, llength + 1, length), length + 2);
            return item;
        } else if (firstByte == hex"c0") {
            //accesslist not support
            bytes memory data;
            data = bytes("");
            item = RLPItem(data, 2);
            return item;
    ...
    function toUintX(bytes memory _bytes, uint256 _start) public pure returns (uint256) {
